...
int elaborated_print_and_check_results(
		...
		result_t* enc_data_out_rec, result_t* enc_expected, const float enc_allowed_precent_diff, int do_print) {
    int err_cnt = 0;
    ...
    for (int sigIdx = 0; sigIdx < NUM_SIGNALS; sigIdx++) {
		...
		std::cout << "--------------------------------------------------------------------------" << std::endl;
		std::cout << "TX (decoder):" << std::endl;
		err_cnt += single_print_and_check_results(&enc_data_out_rec[sigIdx*n_channel], &enc_expected[sigIdx*n_channel], n_channel, enc_allowed_precent_diff, do_print);
		std::cout << "==========================================================================" << std::endl;
    }
//    std::cout << "--------------------------------------------------------------------------" << std::endl;
...
//			tx_data = rand () % M_in;
			...
			for (int i = 0; i < n_channel; i ++) {
				axis_result_t enc_data_out_tmp;
				enc_data_out >> enc_data_out_tmp;
				enc_data_out_rec[sigIdx*n_channel + i] = enc_data_out_tmp.data;
		...
		sim_err_cnt = elaborated_print_and_check_results(
			simIdx, tx_data_rec,
			enc_data_out_rec, enc_expected_rec, enc_allowed_precent_diff, do_print);
